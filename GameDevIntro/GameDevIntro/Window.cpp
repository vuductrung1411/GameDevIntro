...
Window::Window(HWND hWnd, LPDIRECT3DTEXTURE9 texBrick, LPDIRECT3D9 d3d, LPDIRECT3DDEVICE9 d3ddv,
...
void Window::SetCoordAndSize(float x, float y, int Width, int Height)
{
	this->x = x;
	this->y = y;
	this->Height = Height;
...
void Window::DebugOut(const wchar_t* fmt, ...)
{
	wchar_t s[4096];
	VA_PRINTS(s);
	OutputDebugString(s);
...
void Window::DebugOutTitle(const wchar_t* fmt, ...)
{
	wchar_t s[1024];
	VA_PRINTS(s);
...
void Window::InitDirectX()
{
	...
	this->Width = r.right + 1;
	this->Height = r.bottom + 1;
...
void Window::LoadResources(Brick brick)
{
...
void Window::Render(Brick brick, Brick br)
{
	if (this->d3ddv->BeginScene())
	{
		...
		D3DXVECTOR3 p(brick.coords.x, brick.coords.y, 0);
		this->spriteHandler->Draw(texBrick, NULL, NULL, &p, D3DCOLOR_WHITE);

		D3DXVECTOR3 p2(br.coords.x, br.coords.y, 0);
		this->spriteHandler->Draw(texBrick, NULL, NULL, &p2, D3DCOLOR_WHITE);

		DebugOutTitle(L"%s (%0.1f,%0.1f) v:%0.1f", WINDOW_TITLE, brick.coords.x, brick.coords.y, brick.vx);
...
int Window::Run(Brick brick, Brick br)
{
	MSG msg;
	int done = 0;
...
HWND Window::CreateNewWindow(HINSTANCE hInstance, int nCmdShow, int ScreenWidth, int ScreenHeight)
{
	WNDCLASSEX wc;
